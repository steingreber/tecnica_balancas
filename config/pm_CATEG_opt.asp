...
'NOME DO ARQUIVO...: pm_CATEG_opt.asp
...
function go2url() {
window.location = "pm_CATEG.asp";
}
function Validator(theForm)
{
  if (theForm.cp_A01CATEG.value == "")
  {
    alert("O campo A01CATEG não pode ser vazio!");
...
If acao = "1" Then %>
                            <table align="center" border=1 cellspacing=0 width="775" bordercolordark="white" bordercolorlight="black">
                        <form action="pm_CATEG_opt.asp?acao=4&tp=1" method="post" name="frmInc" enctype="multipart/form-data" id="frmInc" onSubmit="return Validator(this);">
                           <input type="hidden" name="tp" value="1">
                               <tr>
                                ...
                                <tr>
       <TD><P><span class="aFontePT">A01CATEG<br></span><INPUT class=ud_caixa style="WIDTH:360px" name=cp_A01CATEG></P></TD>
                        </form>    
                            </table>
<% ElseIf acao = "2" Then %>
<%
...
sSel = "Select * From CATEG Where A00ID =" & sNot
...
%>
                            <table align="center" border=1 cellspacing=0 width="775" bordercolordark="white" bordercolorlight="black">
                        <form action="pm_CATEG_opt.asp?acao=4&tp=2&sF=<% = sNot %>" method="post" name="frmInc" enctype="multipart/form-data" id="frmInc" onSubmit="return Validator(this);">
                               <tr>
                                    <td colspan="2">
                                        <input type="button" name="news_add" value=".::Cancelar::." class="botao" onClick="go2url()">&nbsp;<input type="submit" name="frmId" class="botao" value=".::Gravar::." height=19></td>
                                </tr>
                                <tr>
       <TD><P><span class="aFontePT">A01CATEG<br></span><INPUT class=ud_caixa style="WIDTH:360px" value='<% = va_A01CATEG %>' name=cp_A01CATEG></P></TD>
                        </form>    
                            </table>
<% ElseIf acao = "3" Then %>
<%
sNot = request.querystring("sF")
sSel = "Delete From CATEG Where A00ID =" & sNot
Set objRS = objConect.Execute(sSel)
response.redirect "pm_CATEG.asp"
%>
<%
ElseIf acao = "4" Then
sNot = request.querystring("sF")
...
If sTipo = "1" Then
sNot = request.querystring("sF")
   sSel = sSel & "INSERT INTO CATEG(A01CATEG)"
   sSel = sSel & "VALUES ('" & va_A01CATEG & "')"
   Set objRS = objConect.Execute(sSel)
   response.redirect "pm_CATEG.asp"
...
'***********************************************
sNot = request.querystring("sF")
   sSel = sSel & "UPDATE CATEG SET "
   sSel = sSel & "A01CATEG = '" & va_A01CATEG & "' "
